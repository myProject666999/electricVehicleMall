...
const fetchBannerList = async () => {
  ...
  try {
    ...
    const res = await request.get('/admin/banner/list', { params })
...
const handleSubmit = async () => {
  ...
  try {
    let res
    if (isEdit.value) {
      res = await request.put(`/admin/banner/update/${form.id}`, submitData)
    } else {
      res = await request.post('/admin/banner/create', submitData)
    }
      ...
      >
        ...
        <el-table-column prop="image" label="图片" width="150">
          <template #default="scope">
            <el-image 
              :src="scope.row.image || 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=banner&image_size=square'" 
              :preview-src-list="[scope.row.image || 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=banner&image_size=square']"
              fit="cover"
